...
function f2(array $digit, $act)
{
    if (!empty($digit) && !empty($act)) { //проверяем обязательные параметры функции
        ...
        foreach ($digit as $key => $val) { // проверяем является ли массив списком со значениями - числами
            // Бессмысленная первая часть проверки. Поскольку передается обыкновенный массив без ключей, в $key всегда будет
            // порядковый номер элемента.
            // В то же время, даже если передадут ассоциативный массив, ключи никак не повлияют на работу функции
...
// Доп. задание: проверить работу функции для строки "аиа" и разобраться в причинах. Либо задать вопросы для понимания
// направления.
